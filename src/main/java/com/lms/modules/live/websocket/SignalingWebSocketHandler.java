 ...
 * {
 *   "type": "JOIN_ROOM|LEAVE_ROOM|OFFER|ANSWER|ICE_CANDIDATE|CHAT_MESSAGE|MUTE_TOGGLE|PARTICIPANT_LIST",
 *   "roomId": "session-id",
 *   "payload": { ... }
 * }
 */
...
public class SignalingWebSocketHandler extends TextWebSocketHandler {
    ...
    private void handleJoinRoom(WebSocketSession session, Long roomId, JsonNode payload) {
        ...
        if (liveSession == null) {
            ...
            try { session.close(CloseStatus.BAD_DATA); } catch (Exception e) {}
            return;
        }
        ...
        if ("GUEST".equals(role)) {
            if (!liveSession.getGuestAccessEnabled()) {
                ...
                try { session.close(CloseStatus.POLICY_VIOLATION); } catch (Exception e) {}
                return;
            }
        } else if ("STUDENT".equals(role)) {
            if (userId != null) {
                boolean isEnrolled = enrollmentRepository.existsByStudentIdAndCourseId(userId, liveSession.getCourse().getId());
                if (!isEnrolled) {
    ...
    private void handleIceCandidate(WebSocketSession session, Long roomId, JsonNode payload) {
        ...
        if (payload.has("candidate")) candidatePayload.set("candidate", payload.get("candidate"));
        if (payload.has("sdpMid")) candidatePayload.put("sdpMid", payload.get("sdpMid").asText());
        if (payload.has("sdpMLineIndex")) candidatePayload.put("sdpMLineIndex", payload.get("sdpMLineIndex").asInt());
    ...
    private void broadcastToRoom(Long roomId, String message, String excludeWsId) {
        ...
        for (String wsId : room.participantIds()) {
            if (excludeWsId != null && excludeWsId.equals(wsId)) continue;
